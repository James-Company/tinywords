...
import { validateSentenceCoachInput, type SentenceCoachInput, type WordGenerationInput } from "../../../src/ai/prompts";
...
export function registerAiRoutes(store: InMemoryStore) {
  async function wordGeneration(
    ...
    const enrichedInput: WordGenerationInput = {
      ...
      learning_focus: input.learning_focus || store.profile.learningFocus,
    ...
    // AI 생성 결과를 learningItems에 저장 (이력 추적용)
    if (result.source === "ai") {
      const newItems = toLeajaItems(result.items);
      store.learningItems.push(...newItems);
    }
  ...
  function sentenceCoach(
    ...
    const nextActionKo = overall === "good"
